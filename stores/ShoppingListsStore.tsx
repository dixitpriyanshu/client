...
const useStoreId = () => STORE_ID_PREFIX + useUser().user?.id; // Added ? to remove error warning, will fix later // TODO: Fix this
...
export const useValuesCopy = (
  ...
  useCell("lists", id, "valuesCopy", useStoreId()), // TODO: Fix this
  useSetCellCallback(
    "lists",
    id,
    "valuesCopy",
...
export const useShoppingListsValues = () =>
  Object.values(useTable("lists", useStoreId()))
    ...
    .map(({ valuesCopy }) => {
      try {
        if (valuesCopy) return JSON.parse(valuesCopy); // Added if statement to remove error warning, will fix later // TODO: Fix this
      } catch {
        return {};
      }
    });
